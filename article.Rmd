...
```{r show simulated test, echo=TRUE}
plot(dfMSTResponse)
```
...
The `ltm` library can be used to fit a number of dichotomous and polytomous IRT models. Its the fastest library that I've found for fitting a Rasch model. First we get some summary statistics from the data, including 
...
# return descriptive stats such as Cronbach's alpha
descript <- ltm::descript(dfTrial, chi.squared = FALSE)

# graph the frequencies of the total scores
dfFreqs <- enframe(descript$items[1:length(descript$items)],
                   name = "testscore", value = "frequency")

ggplot(data = dfFreqs) +
  geom_bar(mapping = aes(x = testscore, y = frequency), stat = "identity") +
  xlim(0, 40)
```
