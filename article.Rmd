...
There is an equivalence between $\theta$ and $b$ such that a candidate with mean ability ($\theta = 0$) would be expected to get item 870 (green) correct with a probability of 0.11. The less difficult item 866 (black) would see a probability of around 0.9 for a correct response. Item 868 (red) is somewhere in between the two in terms of difficulty. You could also read this as stating that candidates who have a $\theta$ of 2.07 show a 50:50 chance of getting item 870 correct.
...
plot(raschmodel, type = "ICC", items = itemNo, plot = TRUE, legend = TRUE)
```
This is the item characteristic curve (ICC), in this case for the fifth item in the trial data, `r itemID` with difficulty $b$ = `r itemDifficulty`.
...
ggplot(plotData, aes(x=theta, y=trueScore)) + 
  ...
  labs(title="Module Characteristic Curve", x="θ", y="True Score")
```
This is already showing us something a little weird. Our D (very easy) module is actually more difficult than our F (moderately hard) module. And the E (moderately easy) module is more difficult at the top end of the scale that the G (very hard) module.
