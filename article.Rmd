...
In this section we'll show how to use the Module Information Function (MIF) to derive cut scores for module progression, and also how to build a scale scoring system for the test.

### Item Information Functions (IIFs)

The IIF can be represented visually like this, using the `plot` function from the `ltm` package:
...
Routing between modules is determined by the information functions for the modules in the next stage. We should aim to place the cut score to be that at which the MIFs cross.


```{r Stage 1 MIF}
plotData <- informationCoefficients(plot(raschmodel, 
                                         type = "IIC", 
                                         items = moduleA1_items, 
                                         plot = FALSE), 
                                    "A")

ggplot(plotData, aes(x=theta, y=information)) + 
  geom_line(aes(colour=module)) +
  labs(title="Module Characteristic Curve", x="θ", y="I(θ)")
```
You can see that the maximum information point Module A is pretty close to $\theta = 0$.

Now lets look at the MIFs for the two stage 2 modules:
...
ggplot(plotData, aes(x=theta, y=information)) + 
  geom_line(aes(colour=module)) +
  labs(title="Module Characteristic Curve", x="θ", y="I(θ)") +
  geom_vline(xintercept = 0.81, linetype="dotted")
```
Here we can see that there is point of intersection between modules B and C. This suggests, that candidates who have a $\hat\theta < 0.81$ in Module A should be routed to Module B, and all others routed to Module C.
