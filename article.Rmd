...
The underlying theoretical basis for MST is the branch of psychometrics known as Item Response Theory, or IRT. The theory proposes that there are *latent traits* (for example language ability) that can be measured using a set of items delivered to the test candidate. These test items are typically scored as correct (1) or incorrect (0) - and it is these dichotomous test types that we focus on here. It is also possible to offer polytomous test types (for example based on Likert scales), but we won't cover those in this article.

The response to the test items can then be used to estimate the candidate's latent ability. This can be done by first fitting an IRT model that can then be used to make predictions about latent ability based on the item response. Once the latent ability estimate has been calculated, this can then be converted into a scale score, an ability summary, or both. The rest of this document provides practical examples of designing a multistage test, fitting an IRT model and calculating scores.
...
* The response probability increases with the ability level using the item response function desribed above

The item response function is a logistic function with a characteristic sigmoid (or 'S' shape), and can be plotted for each item. Examples of these Item Characteristic Curves (ICCs) are given here:
...
The slope of the curve indicates how discriminating the item is. An item that is very discriminating will have a steep curve; one that is less discriminating will have a flatter curve. Highly discriminating items have more information. Ideally you want to have many discriminating items to be able to provide a lower error in your ability estimate. In general, the more items you are able to trial with the more information (and less error) you will have.
...
When designing a multistage test, its important to ensure that items are written to the target ability that you want to measure.

Modules need to be constructed in such a way that they target a specified ability range. We'll see later on how a Module Information Function can be used to help with module routing decisions. Also, routing needs to be designed with overlapping items between adjacent modules.
...
In the pre-trial stage, it is advisable to run simulations to help us answer these questions about test design. It's always less expensive to make modifications to a design early on in the process, before significant investment in item writing and trialling has been made. The ability to simulate a test at the design phase can be used to understand the constraints of the design at an early stage and make changes in a low-cost, low-risk way. In this section we'll show how the `mstR` R package can be used to run simulations for a MST design. First we'll discuss the data sources that we'll be using.
...
I mentioned earlier that multistage test designs comprise a number of modules arranged into stages. The question then becomes, how many stages do I need, and how many modules in each stage do I want? We saw earlier an example of a simple 1-2-4 design.
...
Assuming the 1-2-4 pattern shown above, a simulation for the test design can be created from following elements:

* a simulated item bank
* the modules and the items they contain
* the routing rules in the transition matrix

Average difficulty levels for each module are shown below (where $\bar{b}$ is the mean difficulty of the module, and a standard deviation of 1 is assumed)

![MST Design](images/module-items.png)
...
We'll end up with a matrix of 68 items, each with randomised difficulty parameters. A test will comprise 30 items (12 + 8 + 10).
...
The final part of the design activity is construction of the routing paths and cut-offs used for next module selection. This is done by creating an $R \times R$ *transition matrix* which shows the valid paths to modules in the next stage (where $R$ is the number of modules). A value of 1 in the `routing` matrix in the code sample below indicates that a path exists from module $i$ to module $j$. To use the example below the rule `routing[1, 2:3] <- 1` means module 1 routes to modules 2 and 3.

In addition to the routing paths, a set of cut-off criteria must be given. These are expressed in terms of $\hat{theta}$ using the `cutoff` matrix. To use an example here, the rule `cutoff[2,] <- c(-Inf, 0.0)` means that candidates with ability estimates from minus infinity to zero will be routed to module 2. The rule `cutoff[3,] <- c(0.0, Inf)` means that candidates with ability estimates from zero to plus infinity will be routed to module 3.
...
For this simuluted candidate with a true $\theta$ = `r dfMSTResponse$trueTheta`, the estimated ability $\hat{\theta}$ = `r dfMSTResponse$thFinal`. Note that the output from the `randomMST` function uses the module and item indices (rather than names), so you will want to map these back to a name for ease of interpretation.

The Module Information Function is used to determine the 'best' (that is the most informative) module to route to in the next stage. In this example the selected modules are `r dfMSTResponse$selected.modules`. A plot can be made as follows, showing the highlighted route through the multi-stage test:
...
To run the simulations we'll create a set of 7,650 simulated candidates with ability range -4 to 4 (stepped in increments of 0.5). Then we'll repeat for each of the six designs above, with the same item bank.
...
### Module A Mapping Function, $g_{A1}()$

First we use a polynomial regresssion to fit a set of coefficients to the Rasch model.

```{r Module A True Scores, echo=FALSE}
df_moduleA_TS <- trueScoreSummary(plot(raschmodel, type = "ICC", items = moduleA1_items, plot = FALSE))

# use a quintic polynomial regression model to make predictions
lm_moduleA_scores_poly <- lm(theta ~ poly(trueScore, degree=5, raw=TRUE), data = df_moduleA_TS)

# setup the score scale for module A
moduleA_TSRange <- data.frame(
  trueScore = c(seq(0,20)),
  theta = c(rep(0, 21))
)

# plot the theta/true score curve from the Rasch model
plot(x=df_moduleA_TS$theta, y=df_moduleA_TS$trueScore, 
     type="l", col = "coral1", main="Module A", xlab="θ", ylab="True Score")

# overlay the polynomial mapped true score points
scaleScores_moduleA_poly <- predict(lm_moduleA_scores_poly, moduleA_TSRange)
moduleA_TSRange$theta <- unname(scaleScores_moduleA_poly)
points(x=moduleA_TSRange$theta, y=moduleA_TSRange$trueScore, col="cornflowerblue", pch=4)

legend(2, 9, legend=c("MCC", "Quintic Poly Function"),
  col=c("coral1", "cornflowerblue"), lty=1:2, cex=0.8)
...
The Avellain developers, when they want to calculate the ability for a candidate could use such a function to map from the number correct (NC) score to an estimated theta for candidate $i$: $\hat\theta_i$. For example, using R:
...
So, a candidate who scores 13 out of 20 on module A would have a $\hat\theta$ = `r gA1(13)`.
...
plot(x=df_moduleA_TS$theta, y=df_moduleA_TS$trueScore, 
     type="l", col = "coral1", main="Module A", xlab="θ", ylab="True Score")
moduleA_TSRange$theta <- gA1(moduleA_TSRange$trueScore)
points(x=moduleA_TSRange$theta, y=moduleA_TSRange$trueScore, col="purple", pch=4)
legend(2, 9, legend=c("MCC", "gA1()"),
  col=c("coral1", "purple"), lty=1:2, cex=0.8)
```
We can see that it is a good fit, both in the middle of the range, and importantly at the upper and lower bounds.

### Module D-H Mapping Functions, $g_{D1}(), g_{E1}(), g_{F1}, g_{G1}(), g_{H1}()$

```{r}
df_moduleD_TS <- trueScoreSummary(plot(raschmodel, type = "ICC", items = moduleD1_items, plot = FALSE))
lm_moduleD_scores_poly <- lm(theta ~ poly(trueScore, degree=5, raw=TRUE), data = df_moduleD_TS)
gD_coeffs <- unname(coef(lm_moduleD_scores_poly))

df_moduleE_TS <- trueScoreSummary(plot(raschmodel, type = "ICC", items = moduleE1_items, plot = FALSE))
lm_moduleE_scores_poly <- lm(theta ~ poly(trueScore, degree=5, raw=TRUE), data = df_moduleE_TS)
gE_coeffs <- unname(coef(lm_moduleE_scores_poly))

df_moduleF_TS <- trueScoreSummary(plot(raschmodel, type = "ICC", items = moduleF1_items, plot = FALSE))
lm_moduleF_scores_poly <- lm(theta ~ poly(trueScore, degree=5, raw=TRUE), data = df_moduleF_TS)
gF_coeffs <- unname(coef(lm_moduleF_scores_poly))

df_moduleG_TS <- trueScoreSummary(plot(raschmodel, type = "ICC", items = moduleG1_items, plot = FALSE))
lm_moduleG_scores_poly <- lm(theta ~ poly(trueScore, degree=5, raw=TRUE), data = df_moduleG_TS)
gG_coeffs <- unname(coef(lm_moduleG_scores_poly))

df_moduleH_TS <- trueScoreSummary(plot(raschmodel, type = "ICC", items = moduleH1_items, plot = FALSE))
lm_moduleH_scores_poly <- lm(theta ~ poly(trueScore, degree=5, raw=TRUE), data = df_moduleH_TS)
gH_coeffs <- unname(coef(lm_moduleH_scores_poly))
```

```{r Output g Functions}
panel1_coeffs <- data.frame(t(gA_coeffs))
panel1_coeffs <- rbind(panel1_coeffs, data.frame(t(gD_coeffs)))
panel1_coeffs <- rbind(panel1_coeffs, data.frame(t(gE_coeffs)))
panel1_coeffs <- rbind(panel1_coeffs, data.frame(t(gF_coeffs)))
panel1_coeffs <- rbind(panel1_coeffs, data.frame(t(gG_coeffs)))
panel1_coeffs <- rbind(panel1_coeffs, data.frame(t(gH_coeffs)))
panel1_coeffs$moduleID <- c('A1', 'D1', 'E1', 'F1', 'G1', 'H1')
panel1_coeffs <- panel1_coeffs[, c(7, 6, 5, 4, 3, 2, 1)]

# OUTPUT a CSV file too...
write_csv(panel1_coeffs, "../data/scoring-functions-panel1.csv")
```
...
ggplot(itemPlot, aes(x=theta, y=information)) + 
  geom_line() +
  labs(title="Item Information Function", x="θ", y="I(θ)")
```
...
## Routing and Cut-Scores

Routing between modules can then determined by cut scores derived from the intersection of information functions for the modules in the next stage. We should aim to place the cut score to be that at which the MIFs cross.
...
modIIF <- inner_join(iifB, iifC, by = "theta") %>%
  rename(moduleB = information.x, moduleC = information.y) %>%
  dplyr::select(theta, moduleB, moduleC) %>%
  arrange((abs(moduleB - moduleC))) %>%
  head(n = 1)
cutTheta <- modIIF$theta

ggplot(plotData, aes(x=theta, y=information)) + 
  geom_line(aes(colour=module)) +
  labs(title="Module Information Function", x="θ", y="I(θ)") +
  geom_vline(xintercept = cutTheta, linetype="dotted")
```
Here we can see that there is point of intersection between modules B and C where $\hat\theta = $`r cutTheta`. If we read off the true score value that is equivalent for this theta estimate in Module A, then we can use this as the cut score. Below this value the candidate should be routed to Module B, above it they should be routed to Module C.

We can then repeat the exercise for the stage 3 modules.

```{r Stage 3 MIFs}
plotData <- rbind(
  informationCoefficients(plot(raschmodel, 
                               type = "IIC", 
                               items = moduleD1_items, plot = FALSE), 
                          "D"),
  informationCoefficients(plot(raschmodel, 
                               type = "IIC", 
                               items = moduleE1_items, plot = FALSE), 
                          "E"),
  informationCoefficients(plot(raschmodel, 
                               type = "IIC", 
                               items = moduleF1_items, plot = FALSE), 
                          "F"))
...
plotData <- rbind(
  ...
  informationCoefficients(plot(raschmodel, 
                               type = "IIC", 
                               items = moduleG1_items, plot = FALSE), 
                          "G"))
