...
Here we create in R a set of items `it.MST` that will be used in the analysis with `mstR`. We'll be using randomly generated item difficulties drawn from a normal distribution:
...
```{r test-design-params, echo=FALSE}
designParams <- read_csv('mst-designs.csv', skip = 1) %>%
  clean_names() %>%
  dplyr::select(-design, -total)

knitr::kable(
  designParams, booktabs = TRUE,
  caption = 'MST Design paramaters (modules sizes and mean difficulties).'
)
```
...
```{r MST designs}
mstDesign <- function(stage1, stage2, stage3,
...
To run the simulations we'll create a set of simulated candidates with an ability range between -4 and 4 (stepped in increments of 0.5). Then we'll repeat for each of the six designs above, with the same item bank.
...
After making final decisions about test design, test development can start in earnest. In this part we describe what needs to be done before a MST can be finalized.
...
In general, the test designers will want to ensure that each module contains a balance of items of varying difficulty within the target ability range for each module. The Common European Frame of Reference (CEFR) is used to assign items into different ability levels (A1-C2). In our example, the 12 items in Module A cover all levels of the CEFR, as in Figure 9.
...
You can see in Figure 11 that some of the outlier items have a very large divergence from the mean; it is best to remove these items from the item bank as they have too large an error to be used in a live test.
...
Something that we see here that is odd is that our module E (easy) is actually a little bit more difficult than our module F (hard). Also all of the modules are quite similar in terms of difficulty (the MCCs are quite close to each other), and moreover the lines are quite flat - there is little discrimination between them. Again we may want to look at moving some items between modules, or rejecting items with poor discrimination,
...
We could provide candidates with their $\hat{\theta}$ at test end but most candidates and testing organisations do not like theta values and the concept of zero as the population mean and negative ability values are difficult for most people to conceptualise. We therefore need to convert the theta value into a scale score. This usually involves fitting the theta to a range that is chosen by the testing body and acceptable for its stakeholders (for example a score in the range 20-80).
...
An further approach that is interesting is the use of the D-Scoring method (see @han2019developing). We have not had time to undertake a full investigation of the D-Scoring method, but it is certainly something we intend to look at in a future article.
...
The low p-values indicate that this is a useful model for our use. From here we pull out the coefficients (terms) that will be used for the polynomial function.
...
In order to calculate the ability for a candidate, such a function can be used to map from the number correct (NC) score to an estimated theta for candidate $i$, $\hat\theta_i$. The function $g_A()$ that will be used for module A could be written in R function like this:
