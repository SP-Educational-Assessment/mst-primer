...
### Running Repeated Simulations to find RMSE

We can run repeated simulations varying the module topology, number of items per module, mean difficulty levels, or some other aspect of the model. By calculating the RMSE we can see which set of parameters is optimal.
...
The `ltm` library can be used to fit a number of dichotomous and polytomous IRT models. Its the fastest library that I've found for fitting a Rasch model. Before doing that we get some summary statistics from the data, including the Cronbach's alpha measure of covariance. Cronbach's alpha is a function of the number of items in a test, the average covariance between pairs of items, and the variance of the total score. In our test we've split our items into different modules according to the skill that is being measured, so it makes sense to calculate alphas for each module, although we could also do so by stage. We need to make sure that we remove any rows containing NA values before calculating Cronbach's alpha.
...
# its sometimes useful to see the the average covariances between pairs of items and compare this with the variance of the total score.
# this can be done on a per-module basis using the Cronbach's alpha measure of covariance
# to usefully use this we'll need to work out this measure on a per-module basis
...
alphasModuleA <- ltm::cronbach.alpha(dfModuleAItems)
alphasModuleB <- ltm::cronbach.alpha(dfModuleBItems)
alphasModuleC <- ltm::cronbach.alpha(dfModuleCItems)
alphasModuleD <- ltm::cronbach.alpha(dfModuleDItems)
alphasModuleE <- ltm::cronbach.alpha(dfModuleEItems)
alphasModuleF <- ltm::cronbach.alpha(dfModuleFItems)
alphasModuleG <- ltm::cronbach.alpha(dfModuleGItems)

moduleNames <- c('A', 'B', 'C', 'D', 'E', 'F', 'G')
alphas <- c(alphasModuleA$alpha, alphasModuleB$alpha, alphasModuleC$alpha,
